...
public final class ParserTester {
    ...
    private enum Mode {
        SUFFIX("parseObjectPostfix", "toStringPostfix", new Dialect(
                "%s",
                "%s.0",
                (op, args) -> "(" + String.join(" ", args) + " "
                        + (op.startsWith(":") ? op.substring(1) : op) + ")"
        )),
        INFIX("parseObjectInfix", "toStringInfix", new Dialect(
                "%s",
                "%s.0",
                (op, args) -> switch (args.size()) {
                    case 1 -> op.startsWith(":") ? "(" + args.get(0) + " " + op.substring(1) + ")" : op + " " + args.get(0);
                    case 2 -> "(" + args.get(0) + " " + op + " " + args.get(1) + ")";
    ...
    private static Expr.Cata<Parsed> createCata(final ToIntFunction<String> priorities) {
        return Expr.cata(
                name -> priority -> name,
                value -> priority -> "%d.0".formatted(value),
                name -> priority -> name,
                (name, args) -> priority -> {
                    switch (args.size()) {
                        case 1:
                            final String arg = args.get(0).convert(Integer.MAX_VALUE);
